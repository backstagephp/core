...
class ContentFieldValue extends Pivot
{
    ...
    {
        $fieldClass = \Backstage\Fields\Facades\Fields::resolveField($this->field?->field_type ?? '');

        if ($fieldClass) {
            if (in_array(\Backstage\Fields\Contracts\HydratesValues::class, class_implements($fieldClass))) {
                $instance = app($fieldClass);

                return $instance->hydrate($this->value, $this);
            }
        }

        trigger_error('use hydrate() of field instead.', E_USER_DEPRECATED);
