                                            ...
                                            ->options(
                                                Language::active()
                                                    ...
                                                    ->mapWithKeys(function ($languages, $countryName) {
                                                        $countries = once(fn() => Language::active()->where('code', 'LIKE', '%-%')->distinct(DB::raw('SUBSTRING_INDEX(code, "-", -1)'))->count());

                                                        return [
                                                            $countryName => $languages->mapWithKeys(fn($language) => [
                                                                $language->code => '<img src="data:image/svg+xml;base64,' . base64_encode(file_get_contents(base_path('vendor/backstage/cms/resources/img/flags/' . explode('-', $language->code)[0] . '.svg'))) . '" class="inline-block relative w-5" style="top: -1px; margin-right: 3px;"> ' . localized_language_name($language->code) . ($countries > 1 ? ' (' . $countryName . ')' : ''),
                                                            ])->toArray(),
                                                        ];
                                                    })
                                            )
            ...
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\BulkAction::make('translate')
                        ...
                        ->form(function () {
                            $countries = Language::active()->where('code', 'LIKE', '%-%')->distinct(DB::raw('SUBSTRING_INDEX(code, "-", -1)'))->count();
                                ...
                                Select::make('languages')
                                    ...
                                    ->options(
                                        Language::active()
                                            ->get()
                                            ->sort()
                                            ->groupBy(fn($language) => str($language->code)->contains('-') ? localized_country_name($language->code) : __('Worldwide'))
                        ...
                        ->before(function () {
                            /**
                             * Force queue based translating due to translating time
                             */
                            TranslateContentAction::setTranslateOnSync(false);
                        })
                        ->after(function () {
                            Notification::make()
                                ->title(__('Translation started'))
                                ->body(__('Translations will be processed in the background.'))
                                ->success()
                                ->send();
                        }),
